...
llvmir2riscm.logger.info(
    ...
    public void runOnFunc(AsmFunction function) {
        llvmir2riscm.logger.info("color func: "+ function);
    ...
    private void build() {
        ...
        for (AsmBlock block : curFunc.blocks) {
            Set<Register> lives = block.liveOut;
    ...
    private void rewriteProgram() {
        ...
        for (Register reg : spilledNodes) {
            reg.stackOffset = new RawStackOffset(curFunc.stackUse.pointed);
            curFunc.stackUse.pointed += 1;
        }
            ...
            while (it.hasNext()) {
                ...
                for (Register use : inst.uses()) {
                    ...
                    if (!inst.defs().contains(use)) {
                        if (inst instanceof AsmMoveInst && inst.rd.stackOffset == null) {
                            ...
                            AsmBaseInst loadInst = new AsmLoadInst( inst.rd, PhysicalReg.reg("sp"), use.stackOffset, null);
                            it.set(loadInst);
                        } else {
                            VirtualReg temp = new VirtualReg();
                            AsmBaseInst loadInst = new AsmLoadInst( temp, PhysicalReg.reg("sp"), use.stackOffset, null);
                ...
                for (Register def : inst.defs()) {
                    ...
                    if (inst instanceof AsmMoveInst && inst.rs1.stackOffset == null) {
                        ...
                        AsmBaseInst storeInst = new AsmStoreInst(PhysicalReg.reg("sp"), temp, def.stackOffset, null);
                        it.add(storeInst);
                        introducedTemp.add(temp);
                    }
                }
